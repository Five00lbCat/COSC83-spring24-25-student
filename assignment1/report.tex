...
% Figures live in output/figures/ relative to this .tex file's directory.
% Compile from assignment1/ with: pdflatex report.tex
...
% ─────────────────────────────────────────────────────────────────────────────
\begin{abstract}
We report two parallel investigations in computer vision.
...
gap we attribute to limited compute and distribution mismatch rather than
...
Our architecture follows the broad outline of EDSR~\cite{lim2017enhanced} and
ESPCN~\cite{shi2016real}: deep residual feature extraction followed by
sub-pixel convolution for upscaling.
We add a VGG-based perceptual loss (Bonus~B) and evaluate both the L1-only and
...
Convolution would negate both offsets (flipping $K$ by 180°); we use
...
We generate it analytically from
...
We implement two discrete approximations:
...
\begin{figure}[H]
  ...
  \caption{All Part~1 filters applied to a personally captured photograph
           (\texttt{photo\_01.jpg}, resized to 512\,px long edge).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{../filter_results/test_opencv_comparison_smoothing}
  \caption{Smoothing filter comparison: OpenCV \texttt{cv2.blur} and
           \texttt{cv2.GaussianBlur} vs.\ our implementations at $k=5$.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{../filter_results/test_opencv_comparison_edges}
  \caption{Edge detection comparison: OpenCV \texttt{cv2.Laplacian} and
           \texttt{cv2.Sobel} vs.\ our implementations.
           Both pairs match closely, confirming that our correlation convention
...
The network (Figure~\ref{fig:arch}) processes a LR RGB image
...
We use L1 rather than L2 on features because L2 is dominated by large
...
hundreds of epochs on DIV2K to surpass bicubic; our training budget was roughly
$10\times$ shorter.
Additionally, T4 memory constraints forced us to use $128{\times}128$ patches;
...
In our preliminary experiments with $\lambda=0.1$ the perceptual model
...
(30.22\,dB).
This may seem counterintuitive but arises because our evaluation resizes
...
The hysteresis step in our Canny implementation dilates the current edge set
...
With our budget the network has processed approximately $50 \times 800 = 40{,}000$
...
We implemented five classical image filters from scratch and verified them
...
For super-resolution we trained a 16-block residual CNN with PixelShuffle
